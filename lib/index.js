...
const execFileProm = util.promisify(child_process_1.execFile);
exports.job = {
    slackMessage: (message) => () => slack_1.default.sendMessage(message),
    buildReact: (reactAppRootPath) => () => buildReact_1.default(reactAppRootPath),
    uploadDirToS3: (dirPath, credentials) => () => s3_folder_upload_1.default(dirPath, credentials),
    executeFile: (filePath) => () => execFileProm(filePath)
};
exports.init = (hookerOptions) => {
    ...
    webhooks.on("push", async (hook) => {
        ...
        if (hook.payload.ref === hookerOptions.gitRef) {
            ...
            try {
                ...
                slack_1.default.sendMessage(`Hooker job ${hookerOptions.name} failed with message ${e.message}`);
            }
        }
    });
